...
class ChannelRegistry {
	...
	static register(worker, workerId, cvs) {
		...
		for (let [w, cvsOfw] of this.workerToCvs) {
			...
			if (sameCvs.size > 0) {
				...
				const lock = new SharedArrayBuffer(1);
				w.postMessage({ 'command': 'connect', 'cvs': sameCvs , lock }, [channel.port2]);
				worker.postMessage({ 'command': 'connect', 'cvs': sameCvs , lock }, [channel.port1]);
	...
	static synchronizePostMessage(worker, message) {
		port.postMessage({ ...message, lock });
	}
    /**
     * 从两个cvs中获得相同的cv
     */
