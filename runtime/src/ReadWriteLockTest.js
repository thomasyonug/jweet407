...
class __Desk {
    static writeLock_$LI$() {
        if (Desk.reentrantReadWriteLock == null) {
            Desk.reentrantReadWriteLock = new ReentrantReadWriteLock();
        }
        if (Desk.writeLock == null) {
        Desk.writeLock = Desk.reentrantReadWriteLock.writeLock();
    }return Desk.writeLock; }
    static readLock_$LI$() { 
        if (Desk.reentrantReadWriteLock == null) {
            Desk.reentrantReadWriteLock = new ReentrantReadWriteLock();
        }
        if (Desk.readLock == null) {
        Desk.readLock = Desk.reentrantReadWriteLock.readLock();
    }return Desk.readLock; }
    static condition_$LI$() { if (Desk.condition == null) {
        Desk.condition =  Desk.writeLock.newCondition();
...
class Cook extends WebWorker {
    constructor() {
        super(...arguments);
        this.__captured_cvs = { 'Desk.count': Desk.count, 'Desk.writeLock': Desk.writeLock, 'Desk.food_flag': Desk.food_flag, };
                    ...
                    while ((true)) {
                        {
                            ...
                            Desk.writeLock_$LI$().lock();
                            {
                                if (Desk.count === 0) {
                                    //Comm.unsync((Desk.lock_$LI$()));
                                    Desk.writeLock_$LI$().unlock();
                                    break;
                                }
                                else {
                                    if (Desk.food_flag === 1) {
